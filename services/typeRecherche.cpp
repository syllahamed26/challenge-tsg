...
std::vector<int> typeRecherche::recherche_locale_voisinage_variable(const std::vector<City>& cities, std::vector<int> &s, std::vector<int> &neighbor, typeSolution &typeSolution) {
    ...
    for (int i = 0; i < score.number_of_neighbors(s.size()); i++) {
        std::vector<int> solutionCourante = recherche(s, neighbor, cities, typeSolution);
...
std::vector<int> typeRecherche::perturber_solution(const std::vector<int>& solution, int force) {
...
std::vector<int> typeRecherche::recherche_locale_itere(const std::vector<City>& cities, const std::vector<int>& s, std::vector<int>& voisinage, int critere_arret, typeSolution &typeSolution) {
    std::vector<int> solution = s;
    int iterations = 0;

    while (iterations < critere_arret) {
        std::vector<int> meilleureSolution = recherche_locale_voisinage_variable(cities, solution, voisinage, typeSolution);
